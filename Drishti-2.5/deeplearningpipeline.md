...
The [50-frame SemanticKITTI replay summary](runs/replay_seq00/summary.json) reports a 100 ms frame budget, 50/50 deadline misses, 280.3 ms steady-state processing p50, and 315.3 ms steady-state processing p99 on its recorded x86_64 host. Its [per-frame timings](runs/replay_seq00/frames.jsonl) show approximately 180-205 ms in the `mapping_ms` stage in the first few steady frames, versus approximately 25 ms in ground extraction. These measurements are not vehicle end-to-end latencies or a benchmark of other hardware; they do show that adding a GPU solely for FRNet will not resolve the present CPU mapping bottleneck. The [replay manifest](runs/replay_seq00/manifest.json) explicitly lists no temporal fusion, no motion estimation, unavailable deskew, and no real-time claim.
